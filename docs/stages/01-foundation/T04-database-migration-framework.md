...
`etl_runs` is created here rather than in S02 because every ingestion subtask logs into it from [S02.T01](../02-card-data-and-search/T01-etl-cli-and-raw-cache.md) onwards, `/health` will report the last run, and putting it in the foundation migration means the table exists before the first fetcher is written. Open question O-4 — Drizzle, Kysely or hand-written row types — is closed **in** this subtask by a documented procedure rather than by preference, because the answer depends on what exists at implementation time.

## Scope

- **In scope.** `packages/db/src/migrate.ts` (discovery, validation, application, status), the `schema_migrations` bookkeeping, `0001_foundation.sql`, `pnpm db:migrate` / `pnpm db:status`, `assertSchemaCurrent`, `migrationsHash()` for [S01.T03](T03-test-database-and-fixtures.md)'s template cache, the `@pokesearch/db/testing` re-export with migrations applied, `MIGRATIONS.md`, the O-4 procedure and its outcome, `@pokesearch/db/schema` row types and the drift test.
- **Out of scope.** Tables of later stages (`0002` → [S02.T05](../02-card-data-and-search/T05-cards-schema-migration.md), `0003` → [S03.T01](../03-tournament-meta-and-deck-builder/T01-tournaments-schema-migration.md), `0005` → [S04.T14](../04-game-engine-core/T14-jobs-schema-migration.md), `0006` → [S05.T01](../05-card-rules-base/T01-rules-schema-migration.md)); down-migrations; reading or writing `etl_runs` rows ([S02.T01](../02-card-data-and-search/T01-etl-cli-and-raw-cache.md)); the Postgres migration set ([S08.T03](../08-operations-and-extensions/T03-hosted-postgres-migration-path.md)).
...
|---|---|---|---|
| BR-S01.T04-01 | A migration file is applied inside exactly one transaction with its `schema_migrations` row; on failure nothing of that file persists and the recorded version is unchanged. | `applyOne()` — `db.transaction(..., "immediate")` around `exec(sql)` + the bookkeeping insert | `migrate.spec.ts > a failing migration rolls back` (valid `CREATE` then a syntax error → 0 new tables) |
| BR-S01.T04-02 | An applied migration is immutable: the SHA-256 stored at apply time must match the file on every later run; a mismatch aborts before anything is applied. | `validate()` comparing `schema_migrations.checksum` with `sha256(file)` | `migrate.spec.ts > editing an applied migration aborts` |
| BR-S01.T04-03 | Versions are 4-digit, unique and gapless from 0001; a duplicate, a gap, or an applied file missing on disk aborts before any statement runs. | `discover()` + `validate()` ahead of the first transaction | `migrate.spec.ts > rejects duplicate / gap / missing` |
| BR-S01.T04-04 | `migrate()` is idempotent: a second run applies nothing. Migration SQL is therefore written **without** `IF NOT EXISTS`. | version comparison in `pending()`; `sql-lint` flags `IF NOT EXISTS` | `migrate.spec.ts > second run is a no-op`; lint fixture fails |
| BR-S01.T04-05 | Every SQLite-only statement sits inside `-- @sqlite-only` … `-- @end`, preceded by `-- @postgres: <equivalent or "n/a: reason">`. | `scripts/sql-lint.mjs` requires the annotation | `pnpm check` fails on a fixture with an untagged `CREATE VIRTUAL TABLE … USING fts5` |
...
| BR-S01.T04-07 | No code outside `@pokesearch/db/migrate` executes DDL (`CREATE`, `ALTER`, `DROP`). | `sql-lint` scans `packages/*/src` and `apps/*/src`, allowing only `migrate.ts` and the dialect module | `pnpm check` fails on a fixture with `CREATE TABLE` in `apps/api/src` |
...
|---|---|---|---|---|---|
| `schema_migrations` | C | migration runner (whichever process runs `db:migrate`) | once per file, in that file's transaction | insert-only; `version` is the primary key, so a concurrent double-apply fails | `checksum` = SHA-256 of the file as applied |
...
| `schema_migrations` | U / D | — | never | forward-only; a mistake is corrected by a new migration | manual deletion is unsupported (MIGRATIONS.md) |
| `etl_runs` | C | etl | at the start of every run | insert `status='running'`, `finished_at NULL`; monotonic id | helpers owned by [S02.T01](../02-card-data-and-search/T01-etl-cli-and-raw-cache.md) |
| `etl_runs` | U | etl | at the end of the same run | one update of `finished_at`, `status`, `stats_json`, `error`; a crashed run stays `running` until reconciled | the CHECK pair forbids a half-written update |
| `etl_runs` | R | api (`/health`, ETL history), worker scheduler | on request | read-only | `etl_runs_kind_started_idx` serves "latest per kind" |
| `etl_runs` | D | — | not in this stage | retention proposed for [S08.T01](../08-operations-and-extensions/T01-scheduler.md) | to verify with the user first |

**Commands**

| Command | Arguments | Effect | Exit codes |
|---|---|---|---|
| `pnpm db:migrate` | `[--db <path>] [--to <version>] [--dry-run] [--json]` | applies pending migrations to `$DATABASE_PATH` or `--db` | 0 ok, 1 migration failed, 2 validation failed |
| `pnpm db:status` | `[--db <path>] [--json]` | prints applied and pending versions, the resolved path and SQLite version | 0 up to date, 3 pending, 2 validation failed |
...
**Runner semantics.** (1) Discover `NNNN_<snake_name>.sql`, rejecting any other file name. (2) Validate uniqueness, gaplessness, checksum equality for applied files and presence on disk of every applied version. (3) `currentSchemaVersion` reads `MAX(version)`, treating a missing `schema_migrations` as 0 — which is how 0001 bootstraps itself, creating the table and inserting its own row in one transaction. (4) Per pending file, ascending: `BEGIN IMMEDIATE`, re-read the applied set inside the lock (skip if a concurrent runner just applied it), `exec(sql)`, insert the bookkeeping row, `COMMIT`. (5) On error: `ROLLBACK`, stop, throw `MigrationError` naming version, file and SQLite error. (6) `--dry-run` performs (1)–(3) only.

**Conventions** (`packages/db/MIGRATIONS.md`): `NNNN_<name>.sql`, numbers from the data-model inventory; one concern per file; never edited once applied; no `IF NOT EXISTS`; index names `<table>_<cols>_idx`; a header comment stating purpose, owning subtask and the Postgres note; SQLite-only blocks tagged per BR-S01.T04-05; a file that cannot run in a transaction (a twelve-step rebuild needing `PRAGMA foreign_keys=OFF`, or `VACUUM`) declares `-- @no-transaction` first, must be re-runnable, and needs a review note.
...
**`@pokesearch/db/schema`** exports `SchemaMigrationRow`, `EtlRunRow`, the unions `EtlRunKind` and `EtlRunStatus`, and the `TABLES` descriptor the drift test uses.

**O-4 decision procedure**, timeboxed to half a day. (1) Establish the facts: does `drizzle-orm` ship a driver for `node:sqlite` (not `better-sqlite3`) at implementation time, and does Kysely's SQLite dialect accept a custom driver object? (2) Score each candidate against five criteria: (a) no native dependency; (b) the migrations remain the single source of truth for the schema — a tool that wants its own schema DSL fails this; (c) any codegen is deterministic and runs inside `pnpm check`; (d) a raw-SQL escape hatch exists for FTS5, `json_each` and the dialect module; (e) hot paths (search, job polling) can still use hand-written SQL. (3) Spike the winner for two hours against 0001 (insert + select on `etl_runs`). (4) If no candidate satisfies (b) cheaply, take hand-written row types plus the drift test, which is the recommendation of record: the project writes SQL by hand anyway, and the drift test gives most of the safety at zero coupling. (5) Append the outcome, with the facts observed, as a dated resolution of O-4 in the decision log.

## Implementation steps

1. Write `0001_foundation.sql` exactly as above.
2. Implement `discoverMigrations`, `validate` and `currentSchemaVersion` with unit tests over a fixture directory.
3. Implement `migrate()` with the transaction-per-file loop and `MigrationError`; test through `withTempDb`.
4. Add the checksum rule and the gap/duplicate/missing checks with their specs (BR-S01.T04-02, -03).
5. Add `assertSchemaCurrent`, `pendingMigrations` and `migrationsHash`; register `migrate` as the schema initializer in `vitest.setup.ts` — the re-export [S01.T07](T07-api-skeleton-and-health.md) consumes.
6. Write `scripts/db-migrate.mjs` and `scripts/db-status.mjs` with the documented flags and exit codes; spec them against a temp database.
...
9. Run the O-4 procedure and record the decision; a typed layer, if adopted, goes behind `@pokesearch/db/schema` without changing any consumer import.
10. Write `packages/db/MIGRATIONS.md` (conventions + the six-item review checklist referenced by PORTABILITY.md §10).

## Edge cases and error handling

- **A statement fails halfway through a file** → the whole file rolls back (SQLite DDL is transactional); no bookkeeping row is written and the next run retries from scratch.
- **Two processes call `migrate()` at once** (api with `MIGRATE_ON_START=1` and a manual `pnpm db:migrate`) → `BEGIN IMMEDIATE` serialises them; the loser re-reads inside its lock and skips; a residual race on the insert hits the `version` primary key and is treated as "already applied".
...
- **A migration must disable foreign keys** (the twelve-step rebuild) → `PRAGMA foreign_keys` is a no-op inside a transaction, so the file declares `-- @no-transaction`; the runner applies it statement by statement and records it only on full success.
...
- **`schema_migrations` exists but is empty** (rows deleted by hand) → the version reads 0, so 0001 is retried and fails on "table already exists"; the message names the unsupported recovery path instead of guessing at repair.
- **`--to <version>` below the applied version** → refused, exit 2, with "restore a backup (`pnpm db:backup` output) to go back"; there are no down-migrations.
...
- **Risk — a later stage needs a destructive change** (renaming a populated column). Mitigation: MIGRATIONS.md documents the twelve-step rebuild and `-- @no-transaction`; `pnpm db:backup` runs first, its path named in the file's header.
- **Risk — the checksum rule is too strict during early development**, while 0002–0006 are drafted and re-applied. Mitigation: `--allow-checksum-drift` locally; dropping a temp database is free, and the rule is absolute only outside development.
...
- **Question — retention of `etl_runs`.** Nothing deletes rows today; a full ETL history over years is small but unbounded. Proposal: a retention step in [S08.T01](../08-operations-and-extensions/T01-scheduler.md); the user decides whether to keep everything.
- **Question — should `MIGRATE_ON_START` default to 1 in development?** Decided in [S01.T07](T07-api-skeleton-and-health.md); the safe default recommended here is 0, so that applying a migration stays an explicit act.
...
- `pokemon/src/pokesearch/db/schema.sql` — verified: `PRAGMA journal_mode = WAL;` then `CREATE TABLE IF NOT EXISTS sets (...)`, `cards (...)` with text primary keys, `*_json TEXT` columns and ISO-date comments. Consult for the column conventions later migrations follow.
- `pokemon/ESPECIFICACAO.md` §2.4 — verified: where "no migrations" is declared as a legacy limitation.
- [Data model overview](../../project/04-data-model-overview.md) — the 0001–0008 numbering by domain and the migration conventions this file implements.
