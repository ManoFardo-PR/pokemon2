...
**`apps/api/ROUTES.md` — conventions.** (1) One file per domain under `src/routes/`, each a Fastify plugin registered with its prefix; `system.ts` is the one added here. (2) JSON only, `application/json; charset=utf-8`; HTML belongs to `apps/web`; no template engine, no static mount. (3) Paths `/api/<plural-noun>` in kebab-case with `:id` for a single resource; `/health` sits outside `/api` so an operator can curl it without thinking. (4) Every route declares `schema: { params, querystring, body, response }` built from `@pokesearch/shared`; response schemas are mandatory. (5) Pagination: `?page=1&pageSize=60` → `{ items, page, pageSize, total, hasMore }`. (6) `GET` read-only, `POST` create or action, `PATCH` partial update, `DELETE` remove; no verbs in paths. (7) Timestamps are ISO-8601 UTC; ids are the source ids of the data model. (8) Handlers throw typed errors; responses are never composed by hand. (9) Long work is a job, not a request: `POST /api/jobs` + SSE ([S04.T16](../04-game-engine-core/T16-api-jobs-and-sse.md)), and the SSE route is the one documented exemption from the JSON serializer. (10) The api writes only user-facing tables. (11) No auth and loopback binding (D-007); do not add a header "API key", which would imply a security model the project does not have.
...
- **Risk — `/health` grows into a status dashboard** and gets slow. Mitigation: BR-S01.T07-06 fixes a 100 ms budget and "no transaction"; anything richer belongs to a separate `/api/status` in [S08.T02](../08-operations-and-extensions/T02-operations-dashboard.md).
