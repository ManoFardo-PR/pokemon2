...
- `module` `optimizer/stats.ts` — the paired-difference estimator shared with [S07.T03](T03-sequential-confirmation.md) and [S07.T04](T04-holdout-acceptance-and-versioning.md): `pairedDelta(perOpponent) → { delta, se, n, varByOpponent }`, `pairedCi(stats, z)`, `pairedRho(perOpponent)` — consumed by [S07.T03](T03-sequential-confirmation.md), [S07.T05](T05-optimize-job-orchestration.md)
