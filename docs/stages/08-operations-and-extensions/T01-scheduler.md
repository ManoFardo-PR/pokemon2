...
- `module` `apps/worker/src/maintenance.ts` — the two housekeeping tasks the scheduler owns and that earlier subtasks deferred to it: the nightly `pnpm db:backup` ([S01.T02](../01-foundation/T02-sqlite-database-client.md)'s open question) and the weekly retention sweep of `games` rows ([S04.T14](../04-game-engine-core/T14-jobs-schema-migration.md)'s `GAMES_RETENTION_DAYS`)
