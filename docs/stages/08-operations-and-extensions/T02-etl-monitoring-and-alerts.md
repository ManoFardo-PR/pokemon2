...
- `contract` `GET /api/status` — the richer operational view [S01.T07](../01-foundation/T07-api-skeleton-and-health.md) deliberately kept out of `/health`: last run per kind, open alerts, cache ages, table growth
...
- **Risk — `0009` collides with a migration number another S08 subtask claims.** [S08.T06](T06-llm-assisted-authoring.md) also adds a table in this stage. Mitigation: the numbering here is `0009_ops.sql` and the authoring tables take `0010`; both must be confirmed against `packages/db/migrations/` before the first one is applied, and `project/04-data-model-overview.md`'s table inventory currently stops at 0008 and needs both rows added.
