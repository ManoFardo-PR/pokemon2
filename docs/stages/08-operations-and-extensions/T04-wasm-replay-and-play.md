...
- **Sizing — this file is probably two subtasks, and the first is a contract others need.** Its prose runs past the ~2,400-word guidance in [Conventions](../../project/08-conventions.md) because it defines the event-log format and the `replay` job kind *and* builds the browser feature on top of them. The log format is consumed elsewhere — [S07.T07](../07-deck-optimizer/T07-coach-lost-game-review.md)'s coach reads the same `log_blob` for critical moments — so it arguably belongs earlier and on its own. A split would be "event log format and the replay job kind" (steps 1–5, plus `engine/LOG.md`) and "S08.T04 WASM replay and play" (steps 6–11) depending on it. Not applied here, because renumbering is not this pass's to do; proposed for the user's decision.
