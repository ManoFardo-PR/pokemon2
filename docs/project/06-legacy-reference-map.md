...
| `src/pokesearch/sim/pilot.py`, `policies.py`, `tests/test_pilot.py`, `tests/test_policies.py` | Knowledge model, profile, turn order, `need()`, prompt choice by zone; baseline heuristic. RN-33's real brake is `deck_n - n >= SAFE_DECK` at L447 — refused when the deck would fall below 7 *after* the draw | S06.T01–T04, S04.T11 |
| `src/pokesearch/sim/frozen/` | The freezing policy and the three frozen pilots. **They exist** — `planner_v4.py` (24,516 B), `planner_v7.py` (27,271 B), `planner_v9.py` (29,331 B) — and each carries a `# CONGELADO em …` banner. **Defect to design against**: `planner_v9.py` L30 imports `MAX_CHOICE_SCAN, _attack_recipe, _can_pay, _discards_hand, _is_neutral_stadium, _missing, _turn_is_empty` from the live `policies.py`, which was modified on 2026-09-21, the day after that pilot was frozen. A "frozen" opponent that shares mutable helpers with an edited module is not frozen, so RN-40's "between two measurements only the pilot under test changes" could not be guaranteed. S06.T07 requires a frozen bot to be self-contained and proves it with an import-graph test | S06.T07 |
